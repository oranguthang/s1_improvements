# s1_improvements

`s1_improvements` is a Sonic 1 disassembly-based project focused on gameplay, engine, and quality-of-life improvements implemented from Sonic Retro tutorials.

This repository is built on top of the Sonic 1 disassembly and gradually ports fixes and features from the Sonic Retro hacking guides into one project.

See also:
- Sonic Retro Disassemblies: https://info.sonicretro.org/Disassemblies
- Sonic Retro How-To guides: https://info.sonicretro.org/SCHG_How-to:Guide

## Implemented Sonic Retro tutorials

- [Fix the remember sprite related bug](https://info.sonicretro.org/SCHG_How-to:Fix_a_remember_sprite_related_bug)
- [Have a functional Eggman monitor in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Have_a_functional_Eggman_monitor_in_Sonic_1)
- [Fix the Special Stage jumping physics](https://info.sonicretro.org/SCHG_How-to:Fix_the_Special_Stage_jumping_physics)
- [Improve the fade in/fade out progression routines in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Improve_the_fade_in/fade_out_progression_routines_in_Sonic_1)
- [Fix Scattered Rings Underwater Physics](https://info.sonicretro.org/SCHG_How-to:Fix_Scattered_Rings_Underwater_Physics)
- [Remove the Speed Cap](https://info.sonicretro.org/SCHG_How-to:Remove_the_Speed_Cap)
- [Remove the Roll-Jump Lock](https://info.sonicretro.org/SCHG_How-to:Remove_the_Roll-Jump_Lock)
- [Smooth Rotation in Special Stages](https://info.sonicretro.org/SCHG_How-to:Smooth_Rotation_in_Special_Stages)
- [Collide with water after being hurt](https://info.sonicretro.org/SCHG_How-to:Collide_with_water_after_being_hurt)
- [Skip Score Tallies](https://info.sonicretro.org/SCHG_How-to:Skip_Score_Tallies)
- [Add Extended Camera to Sonic 1](https://info.sonicretro.org/SCHG_How-to:Add_Extended_Camera_to_Sonic_1)
- [Add the Air Roll/Flying Spin Attack](https://info.sonicretro.org/SCHG_How-to:Add_the_Air_Roll/Flying_Spin_Attack)
- [Create a Homing Attack from scratch](https://info.sonicretro.org/SCHG_How-to:Create_a_Homing_Attack_from_scratch)
- [Change Spike behavior in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Change_Spike_behavior_in_Sonic_1)
- [Fix a race condition with Pattern Load Cues](https://info.sonicretro.org/SCHG_How-to:Fix_a_race_condition_with_Pattern_Load_Cues)
- [Fix demo playback](https://info.sonicretro.org/SCHG_How-to:Fix_demo_playback)
- [Fix Accidental Deletion of Scattered Rings](https://info.sonicretro.org/SCHG_How-to:Fix_Accidental_Deletion_of_Scattered_Rings)
- [Fix Ring Timers](https://info.sonicretro.org/SCHG_How-to:Fix_Ring_Timers)
- [Fix the Walk-Jump Bug in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Fix_the_Walk-Jump_Bug_in_Sonic_1)
- [Correct Drowning Bugs in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Correct_Drowning_Bugs_in_Sonic_1)
- [Fix the death boundary bug](https://info.sonicretro.org/SCHG_How-to:Fix_the_death_boundary_bug)
- [Fix the camera follow bug](https://info.sonicretro.org/SCHG_How-to:Fix_the_camera_follow_bug)
- [Fix Song Restoration Bugs in Sonic 1's Sound Driver](https://info.sonicretro.org/SCHG_How-to:Fix_Song_Restoration_Bugs_in_Sonic_1%27s_Sound_Driver)
- [Fix the HUD blinking](https://info.sonicretro.org/SCHG_How-to:Fix_the_HUD_blinking)
- [Fix the Level Select graphics bug](https://info.sonicretro.org/SCHG_How-to:Fix_the_Level_Select_graphics_bug)
- [Fix Spikes Backside Damage in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Fix_Spikes_Backside_Damage_in_Sonic_1)
- [Fix the Title Screen position in Sonic 1](https://info.sonicretro.org/SCHG_How-to:Fix_the_Title_Screen_position_in_Sonic_1)

## Work in progress

- [Add Super Sonic to Sonic 1](https://info.sonicretro.org/SCHG_How-to:Add_Super_Sonic_to_Sonic_1) is currently `WIP` and lives on the `super_sonic` branch for now.

## Disclaimer

Any and all content presented in this repository is presented for informational and educational purposes only.
Commercial usage is expressly prohibited. Sonic Retro claims no ownership of any code in these repositories.
You assume any and all responsibility for using this content responsibly. Sonic Retro claims no responsibility or warranty.
